                            ...
                            @forelse($commandes as $commande)
                                <tr class="text-center border-top">
                                    ...
                                    <td>
                                        ...
                                        @endphp
                                        <span class="badge bg-{{ $color }} bg-opacity-25 text-{{ $color }} fw-semibold">
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('commande.show', $commande) }}" class="btn btn-sm btn-light border text-primary">
                                                Voir
                                            </a>
                                            <a href="{{ route('commande.edit', $commande) }}" class="btn btn-sm btn-light border text-warning">
                                                Modifier
                                            </a>
                                            <form action="{{ route('commande.destroy', $commande) }}" method="POST" onsubmit="return confirm('Supprimer cette commande ?')" class="d-inline">
                                                @csrf
                                                @method('DELETE')
