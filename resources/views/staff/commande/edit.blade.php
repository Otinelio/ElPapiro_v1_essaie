        ...
        <div class="container">
            <div class="card border-0 shadow-sm rounded-4">
                ...
                <div class="card-body px-4 py-4">
                    ...
                    @else
                        <form action="{{ route('staff.commande.update', $commande) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('staff.commande.valider.paiement', $commande) }}"
                                    class="btn btn-success">Valider le payement</a>
                                <form action="{{ route('staff.commande.update', $commande) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-primary">Sauvegarder</button>
                                </form>
                                <a href="{{ route('staff.commande.index') }}" class="btn btn-outline-secondary">Retour</a>
                            </div>
                        </form>
                    @endif
                </div>
