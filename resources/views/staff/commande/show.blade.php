        ...
        <div class="container">
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white d-flex justify-content-between align-items-center py-3 px-4 border-bottom">
                    ...
                    <a href="{{ route('staff.commande.index') }}" class="btn btn-sm btn-outline-secondary">
                        ⬅ Retour à la liste
                    </a>
                </div>
                ...
                <div class="card-body px-4 py-4">
                    <div class="mb-4">
                        ...
                        <ul class="list-group list-group-flush">
                            <li class="list-group-item px-0 py-2"><strong>Nom :</strong>
                                {{ $commande->nom_client ?? $commande->client_name }}</li>
                            <li class="list-group-item px-0 py-2"><strong>Email :</strong>
                                {{ $commande->email_client ?? $commande->client_email }}</li>
                            <li class="list-group-item px-0 py-2"><strong>Téléphone :</strong>
                                {{ $commande->telephone_client ?? $commande->client_phone }}</li>
                            <li class="list-group-item px-0 py-2">
                                <strong>Adresse :</strong> {{ $commande->adresse ?? '-' }}, {{ $commande->ville ?? '-' }},
                                {{ $commande->pays ?? '-' }}
                            </li>
                            <li class="list-group-item px-0 py-2">
                                <strong>Montant :</strong> <span
                                    class="fw-semibold text-dark">{{ number_format($commande->montant_total ?? $commande->prix_total, 0, ',', ' ') }}
                                    FCFA</span>
                            </li>
                            <li class="list-group-item px-0 py-2">
                            ...
                            <li class="list-group-item px-0 py-2">
                                ...
                                @endphp
                                <span
                                    class="badge bg-{{ $color }} bg-opacity-25 text-{{ $color }} fw-semibold px-3 py-1">
                                    ...
                                    @forelse($items as $item)
                                        <tr class="text-center">
                                                ...
                                                <div class="d-flex align-items-center">
                                                    @if ($item->produit->image)
                                                        <img src="{{ asset('assets/produits/' . $item->produit->image) }}"
                                                            alt="{{ $item->produit->nom }}" class="rounded-3 me-3"
                                                    ...
                                                    <div class="text-start">
                                                        <h6 class="mb-1 fw-bold text-dark">
                                                            {{ $item->produit->name ?? 'Produit non disponible' }}</h6>
                                                        @if ($item->produit->description)
                                                            <small
                                                                class="text-muted d-block">{{ Str::limit($item->produit->description, 50) }}</small>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="py-3 fw-bold">
                                                {{ number_format($item->prix * $item->quantite, 0, ',', ' ') }} FCFA</td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-4 text-muted">Aucun produit dans cette
                                                commande</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                                <tfoot class="table-light">
                                    <tr>
                                        ...
                                        <td class="text-center fw-bold py-3">
                                            {{ number_format($commande->montant_total ?? $commande->prix_total, 0, ',', ' ') }}
                                            FCFA</td>
                                    </tr>
                                </tfoot>
                            </table>
